...
async function fetchHyperliquidMark(): Promise<number> {
  ...
  const i = (meta?.universe ?? []).findIndex(
    (u: any) => (u?.name || "").toUpperCase() === "XPL"
  );
  if (i < 0) throw new Error("XPL not found");
  const v = Number(ctxs?.[i]?.markPx);
  if (!isFinite(v)) throw new Error("Invalid markPx");
  return v;
}
...
export default async function handler(req: any, res: any) {
  try {
    ...
    const queryToken =
      (req.query && (req.query.token as string)) || undefined;
    if (
      CRON_SECRET &&
      headerToken !== CRON_SECRET &&
      queryToken !== CRON_SECRET
    ) {
      return res.status(401).json({ ok: false, error: "unauthorized" });
    }

    const [hl, bx] = await Promise.all([
      fetchHyperliquidMark(),
      fetchBingxSpot(),
    ]);
